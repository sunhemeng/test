...
for r in range(loop_start, loop_end + 1):
    ...
    for cb in comb:
        ...
        print(parameters)
        np.savetxt(sim_env.channel_type + '_Hyperparameters_critic_loss_expand_training_Avg_Error' + parameters + '.csv', np.transpose(error_avg),
                        header='Error[sum(-log10(e))]', fmt='0%30.28f')
        np.savetxt(sim_env.channel_type + '_Hyperparameters_critic_loss_expand_training_Abs_Error' + parameters + '.csv', np.transpose(error), header='Error',
                        fmt='0%30.28f')
        np.savetxt(sim_env.channel_type + '_Hyperparameters_critic_loss_expand_training_Avg_Gradient' + parameters + '.csv', grad_overall,
                         header='Average_Gradient of every batch/train cycle', fmt='0%30.28f')
        np.savetxt(sim_env.channel_type + '_Hyperparameters_critic_loss_expand_training_AvgLoss_Critic' + parameters + '.csv', QN.critic_loss_a,
                   header='Average_Loss of every Training from Critic Network', fmt='%30.28f')
        QN.save_weights(sim_env.channel_type + '_Hyperparameters_critic_loss_expand_training_', parameters)
