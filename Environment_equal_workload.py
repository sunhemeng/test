...
class Simulation:
    ...
    def total_error(self, n, a):
        ...
        for k in range(self.S):
            if a[k]!=0:
                ...
                if self.CSI == 1:
                    ...
                    if comp_depend < 0:
                        comp_depend = 0
                m = t2 - ((((ck*a[k]) + self.d) + comp_depend) / self.f)
                if m < 0:
                    m = 0
                    #m[m < 0] = 0
                    # print('m:', m)
                ...
                # Link k error
                total_k = a[k] * (comm_error + comp_err - (
        ...
        self.tasks_prev = np.append(self.tasks_prev[1:], np.multiply(a, ck))
        self.tasks_prev = self.tasks_prev.reshape((self.W - 1 + self.CSI, self.S))
